...
def kirim():
    ...
    h = f"hello nama saya {nama.get()} jenis kelamin {jk.get()} di kelas {kelas.get()} dan saya { stj } mengerjakan project bahasa asing {ba.get()}"
...
formJK = tema.Combobox(frame, textvariable= jk, value=("Laki-laki", "Perempuan"))
formJK.pack(padx=10, fill="x", expand=True)
formJK.current(0)

labelba = tema.Label(frame, text="Bahasa Asing :")
labelba.pack(padx=10, fill="x", expand=True)
ba1 = tema.Radiobutton(frame, value="dkv", text="dkv", variable=ba)
ba1.pack(padx=10, pady=10, fill="x", expand=True)
ba2 = tema.Radiobutton(frame, value="coding", text="coding", variable=ba)
ba2.pack(padx=10, pady=10, fill="x", expand=True)

stj = tema.Checkbutton(frame, variable=setuju, text="Saya setuju mengerjakan projek akhir ..", onvalue=1, offvalue=0)
stj.pack(padx=10, pady=10, fill="x", expand=True)
